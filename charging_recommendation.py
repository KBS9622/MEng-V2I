...
class charging_recommendation(object):
    ...
    def set_EV_data(self, new_EV_data):
        ...
        if start_time_new_EV_data > self.EV_data.iloc[-1, :].name:
            ...
            self.previous_end = self.journey_end[-1, :] # get the last predicted journey end time as a reference to start of charging availability
            if self.journey_end[-1, :] < end_time_new_EV_data - timedelta(hours = self.config_dict['System_consideration_span']):
                self.previous_end = end_time_new_EV_data - timedelta(hours = self.config_dict['System_consideration_span'])
        else:
            #if the EV plug in time is before the end of the last predicted drive cycle
            self.previous_end = [self.EV_data.loc[start_time_new_EV_data, :].name] #plug in time (start_time_new_EV_data) is the starting reference for charging availability
    ...
    def recommend(self):
        ...
        lower_limit = battery_capacity * (self.config_dict['Lower_buffer'] + self.config_dict['Emergency_reserves'])/100 # lower limit can also include the buffer the manufacturers set *if the charge level obtained from EV has not considered that yet
        upper_limit = battery_capacity * (self.config_dict['Upper_buffer']/100) #does not consider the manufacturer upper buffer yet
        ...
        if start_time_slot == end_time_slot:
            ...
            pred.loc[start_time_slot, ['charging', 'journey']] += (start_of_day.ceil(freq='30min') - start_of_day).seconds / 60
