...
def real_data_loading(path, column, seq_len):
  ...
  """    
  ori_data = pd.read_csv(path)[column]
...
def disc_score(ori_data, generated_data):
  metric_iteration = 5
...
if __name__ == "__main__":
  path = './results/csv/Generated_TimeSeries_usingWGANGP_Epoch1800.csv'
  seq_len = 60
  
  ori_data = real_data_loading(path, 'Real', seq_len)
  ori_data = np.expand_dims(ori_data, axis=2)

  generated_data = real_data_loading(path, 'Fake', seq_len)
  generated_data = np.expand_dims(generated_data, axis=2)

  disc_score(ori_data, generated_data)
  pred_score(ori_data, generated_data)